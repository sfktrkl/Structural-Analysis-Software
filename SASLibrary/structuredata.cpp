...
void structureData::on_refresh_released()
{
    ...
    if (Node::numOfDofs-1 != 0){
        for (unsigned int i = 0; i<Node::numOfDofs-1;i++){
            for (unsigned int j = 0; j<Node::numOfDofs-1;j++){
                ui->stiffnessMatrix->setItem(int(i),int(j),new QTableWidgetItem(QString::number(MainWindow::KGlobalMatrix[i][j])));
            }
        }
    }
    ...
    if (Node::numOfDofs-1 != 0){
        for (unsigned int i = 0; i<Node::numOfDofs-1;i++){
                ui->forceMatrix->setItem(int(i),int(0),new QTableWidgetItem(QString::number(MainWindow::FGlobalMatrix[i][0])));
        }
    }
    ...
    if (Node::numOfDofs-1 != 0){
        for (unsigned int i = 0; i<Node::numOfDofs-1;i++){
                ui->deflectionMatrix->setItem(int(i),int(0),new QTableWidgetItem(QString::number(MainWindow::Deflections[i][0])));
        }
    }
